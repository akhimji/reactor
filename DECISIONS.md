...
- **Implementation note (added retrospectively):** Phase 5 must guard against transitioning atoms to `splitting` in the same tick they entered `excited`. Use `excitedSince < currentTick` as the eligibility check. This preserves the 1-tick `excited` window required for renderer buildup animations and is consistent with ADR-016's same-tick deferral pattern for neutrons. Without this guard, phase 5 would immediately consume atoms phase 4 just transitioned, eliminating the observable `excited` state.
...
## ADR-027: Spent atom cleanup

- **Date:** 2026-04-30
- **Decision:** Spent atoms persist in state for a fixed number of ticks after entering the `spent` state, then are removed by phase 7. Default `physics.spentAtomCleanupTicks: 60` (1 second at 60Hz). Tunable via config. Atoms gain an optional `spentAt: number` field populated whenever they transition to `spent` from `splitting` (phase 6) or via collision absorb (phase 4, except B10). Phase 7 removes atoms when `state.tick - atom.spentAt >= spentAtomCleanupTicks`. No event is emitted on cleanup.
- **Context:** Phases 4 and 6 transition atoms to `spent` but never remove them from state. Without cleanup, atom maps grow unbounded across long runs, polluting iteration cost in every phase that walks atoms. Sim spec §4.1.7 mentions cleanup ("spent atoms past cleanup threshold are removed") but does not lock the threshold or the mechanism. We need both before phase 7 lands.
- **Alternatives:**
  - *Never remove spent atoms* — rejected. Maps grow without bound. Phase 4 already excludes spent atoms from collisions, but other future phases (poisoning, scoring sweeps, render-state queries) might iterate atoms and pay O(n) for entries that no longer matter. Determinism is fine either way; this is a perf and hygiene argument.
  - *Remove immediately on transition to `spent`* — rejected. The `spent` state has visual meaning: the renderer's "burned out" / "fade out" frames need a window in which the atom still exists in state to anchor sprite tweens. Removing in the same tick the atom enters `spent` collapses that window to zero.
  - *Per-atom-type cleanup duration* — rejected for v1. Adds tuning surface for no current need. Pu239 cleanup time isn't fundamentally different from U235 cleanup time at the visual level. Revisit if a Site genuinely needs per-type behavior.
  - *Per-Site override of `spentAtomCleanupTicks`* — out of scope for v1; the existing per-Site config layering (spec §9) already covers this if it's needed later.
  - *Emit a `cleanupCompleted` event on removal* — rejected. The prior `atomSpent` or `atomDecayed` event already informed subscribers that the atom's logical lifetime is over. Cleanup is a state-housekeeping detail; emitting on it would force every subscriber to handle two events for one logical end-of-life and creates a confusing "is this atom alive?" question for renderers tracking sprite lifecycles.
- **Consequences:** `Atom` gains `spentAt?: number` per ADR-015 (one fact per field): the tick when this atom entered the `spent` state. Populated by phase 4 (on absorb-spent transition, except B10) and phase 6 (on splitting → spent transition); not populated by phase 7's auto-decay path because that path skips the `spent` state entirely (ADR-028). Phase 7 reads `spentAt` to gate cleanup. If a future phase introduces a new `→ spent` transition, it must populate `spentAt = state.tick` at the transition point, or the atom will linger in state until something else cleans it up. Tests should assert that every atom transitioning to `spent` has `spentAt` set. Atoms with `state === 'spent'` but `spentAt === undefined` are a bug; phase 7 leaves them alone (no crash, but worth a code comment noting the assumption).

---

## ADR-028: `atomDecayed` vs `atomSpent` semantics

- **Date:** 2026-04-30
- **Decision:** Two distinct events for atoms reaching the `spent` state, distinguished by cause. `atomSpent` fires when an atom transitioned from `splitting` to `spent` (post-fission, phase 6) or from `intact` to `spent` via collision absorb (phase 4, except B10) — the atom was *used*. `atomDecayed` fires when an atom transitioned from `intact` to `spent` via timer expiry without being split (phase 7, Pu239 only in v1) — the atom *timed out*. Both result in the same final state. Phase 7 emits `atomDecayed` (not `atomSpent`) for timer-driven decay, and removes the atom from state on the SAME tick the decay event fires — no `spentAtomCleanupTicks` wait, because the atom never had a meaningful "spent" lifetime; it went straight from `intact` to gone.
- **Context:** Sim spec §8.1 defines both events but does not specify which path emits which. Phase 7 handles two distinct kinds of "atom is over": (a) Pu239 timed out without splitting, and (b) atoms that already transitioned to `spent` (by phase 4 or 6) and have aged past the cleanup threshold (ADR-027). Collapsing the two paths into one event would lose a meaningful gameplay signal — Pu239 decay is rare and high-yield, and letting one decay unused is a real failure that deserves visible feedback distinct from "this atom got used."
- **Alternatives:**
  - *Single `atomSpent` event for both paths* — rejected. Renderer would have to inspect atom type and prior state to know whether to play the "used up" animation or the "wasted resource" animation. Per ADR-018 (event payloads are self-contained), pushing that inference into subscribers is exactly the wrong direction.
  - *Emit both `atomSpent` and `atomDecayed` for the timer path* — rejected. Subscribers would have to deduplicate. ADR-014 lets `neutronExpired` shadow `neutronAbsorbed` because they convey different information (lifetime end vs. interaction context); for atom decay there is no second channel of information — the cause is the only fact, and it fits one event cleanly.
  - *Make Pu239 transition through `spent` first, then auto-cleanup the same as any other spent atom* — rejected. Forces the renderer to either (a) play the "used" animation for a wasted atom (visually wrong) or (b) inspect the prior tick's state to suppress the animation (couples renderer to history). Going `intact → gone` directly with `atomDecayed` is the cleaner channel.
- **Consequences:** Phase 7 has two paths with different behaviors: (1) Pu239 timer-driven decay — emits `atomDecayed`, removes atom immediately, never observed in `spent` state. (2) Spent atom cleanup (atoms reached `spent` via phase 4 or 6) — waits `spentAtomCleanupTicks` after `spentAt`, then removes. No event on cleanup (ADR-027). Renderer gets a clean signal: `atomSpent` = used (play "burned out" frames), `atomDecayed` = wasted (play "decayed without splitting" frames, possibly different sound). The `spent` state is observable in state for the cleanup window, allowing sprite tweens to anchor; decayed atoms are never observable in `spent` state, which matches their "this atom never got to fission" gameplay meaning.

---

## ADR-029: Rolling-average criticality — naive recompute over fixed window

- **Date:** 2026-04-30
- **Decision:** Phase 8 computes criticality `k` as a naive recompute over the rolling window. `SimState` carries a `fissionHistory: number[]` of fixed length `criticalityWindow` (default 120). Each tick, phase 8 writes `fissionsThisTick` into `fissionHistory[state.tick % criticalityWindow]` (overwriting the slot from `criticalityWindow` ticks ago), then sums all slots to compute `windowTotal`, then derives `k = windowTotal / (windowSeconds * baselineNeutronRate)`. O(window) per tick = ~120 ops, well within the per-tick budget.
- **Context:** Spec §5.1 specifies a fixed-window rolling average. Phase 8 needs a concrete data structure and update rule. There are at least three viable shapes (naive recompute, circular buffer with running sum, exponential moving average); the choice has determinism and refactor-cost implications, so it should be locked before implementation.
- **Alternatives:**
  - *Circular buffer with running sum* — rejected for v1. Asymptotically better (O(1) per tick) but trivially better at this scale: 120 array sums per tick is invisible in profiling. The implementation is also more error-prone (must subtract the overwritten slot value before adding the new one; drift from accumulated float error over long runs is a real concern). Naive recompute has zero drift because it sums the canonical history each tick.
  - *Exponential moving average (`k_new = α * k_obs + (1 - α) * k_old`)* — rejected. Different decay characteristics from a fixed window: an EMA has unbounded memory of past events with diminishing weight, where the spec asks for a hard cutoff. EMA also makes the "criticality reflects the last N ticks" mental model harder to communicate to players, which matters for the criticality meter UI.
  - *Per-event log with timestamp filter* — rejected. Storing every fission event with its tick and filtering by `tick > state.tick - criticalityWindow` works but trades `O(window)` for `O(events_in_window)`; at peak chain reactions events_in_window can exceed window. The fixed array is bounded.
- **Consequences:** `fissionHistory` is initialized as `new Array(config.criticalityWindow).fill(0)` in `createSimState`. Phase 8 reads `state.pendingEvents` (without consuming) to find atomSplit events for the current tick, sums `neutronsReleased` (ADR-030), writes the slot, sums the array, computes k. Same pattern as ADR-024 (collision-detection deferral): if perf ever demands the swap to a circular buffer, it's a contained change behind the same `phaseRecomputeCriticality` interface — the array semantics and slot-indexing rule are the contract. Future renderer/score reads of `state.criticality` are stable across the swap.

---

## ADR-030: Fission counting semantics — sum of neutronsReleased

- **Date:** 2026-04-30
- **Decision:** The per-tick fission count fed into `fissionHistory` (ADR-029) is the SUM of `neutronsReleased` across all `atomSplit` events emitted in the current tick — not a count of split events. `atomDecayed` events (Pu239 timer expiry) are NOT counted; decay releases zero neutrons. Phase 8 reads `state.pendingEvents` for the current tick and sums `data.neutronsReleased` for entries of `type === 'atomSplit'`. Other event types are ignored for criticality purposes. Phase 8 reads but does not consume or modify `pendingEvents`; phase 10 owns the flush.
- **Context:** Spec §5.1 says `k` is computed from "neutrons produced by fission events" over a rolling window. The phrasing leaves room for two readings: (a) count of splits that happened, or (b) count of neutrons that came out of those splits. Phase 8 needs to pick one and commit. The choice changes the gameplay meaning of `k` — counting events vs. counting neutrons gives U235 (2-3 per split) and Pu239 (2-4 per split) different weights in the criticality average.
- **Alternatives:**
  - *Count atomSplit events* — rejected. Spec §5.1 specifically says "neutrons produced," not "splits occurred." A 4-neutron Pu239 split feeds the chain reaction twice as strongly as a 2-neutron U235 split; counting events flattens that signal, which would make Pu239 less rewarding to fission and break the design intent that high-yield fuel produces stronger chain reactions.
  - *Include `atomDecayed` in the count* — rejected. Decay produces zero neutrons by definition (ADR-028 — Pu239 timer-driven decay is the "atom timed out unused" path). The criticality meter should only respond to actual fission output, not wasted timer expirations. Including decay would make the meter rise on events that have no fuel-output meaning, which is the opposite of what the player should observe.
  - *Hybrid weighting (count splits but apply a per-type yield multiplier)* — rejected. The atom-type yield is already encoded in `neutronsPerSplit` ranges, which produce the per-split neutron counts that phase 4 emits as `neutronsReleased`. Adding a second multiplier on top would double-count and complicate tuning.
- **Consequences:** Phase 8 walks `state.pendingEvents` once per tick; the cost is O(events_this_tick), which is bounded (peak chain-reaction tick produces dozens of events at most). The dependency graph is: phase 4 emits `atomSplit` with `neutronsReleased` → phase 5 spawns those neutrons → phase 8 reads the same `atomSplit` event payloads in the same tick to update criticality. Phase 8 does not need to walk the neutron map or atom map; the events are the canonical fission record. Phase 10 (flush) runs after phase 8, so events are still in the queue when phase 8 reads them. If a future phase introduces a non-event channel for fissions (e.g., direct state manipulation by an admin command), it must also write to a counter that phase 8 reads, or fission output will silently fail to register. ADR-018 (event payloads are self-contained) makes the event channel sufficient; phase 8 does not need to re-derive the count from atom state, which would be more complex and tied to internal representation.

---

## ADR-031: Criticality event emission — `tick` every tick, `criticalityZoneChanged` on change

- **Date:** 2026-04-30
- **Decision:** Phase 8 emits two distinct events per tick. (1) `tick` — emitted EVERY tick, payload `{ criticality: k, zone: newZone }` per spec §8.1. This is the per-tick criticality stream that drives the criticality meter UI. (2) `criticalityZoneChanged` — emitted ONLY when `previousZone !== newZone`, payload `{ previousZone, newZone, k }` per spec §8.1. This drives discrete reactions (zone-change flash, zone-specific alarm sound). Phase 8 derives `previousZone` from `state.criticality?.zone ?? 'extinct'` — the implicit starting zone before phase 8 first runs. The first tick's transition from `extinct` to whatever zone the initial state computes as fires `criticalityZoneChanged` (e.g., `extinct → nominal` if the run starts at sustaining k); this is the "reactor coming online" signal and is acceptable.
- **Context:** Spec §8.1 defines both events but doesn't specify their emission cadence. Renderers have two distinct needs: a continuous stream for meters and a discrete signal for boundary-crossing reactions. Conflating them — for example, only emitting `tick` and forcing subscribers to compare consecutive `tick` payloads to detect zone changes — would push state-tracking complexity into every subscriber and lose the per-event semantic clarity that ADR-018 protects.
- **Alternatives:**
  - *Single `tick` event; subscribers diff zones across consecutive events* — rejected. Renderers would need to maintain a "last seen zone" cache to detect changes, which couples them to the per-tick event stream's continuity (a missed event corrupts the cache). The discrete signal is a different fact than the continuous one; emitting them as separate events lets each subscriber pick the channel that matches its need.
  - *Emit `criticalityZoneChanged` every tick (with `previousZone === newZone` when no change)* — rejected. Cluttered: on the no-change path the event carries no information (`previousZone === newZone === k`), and the event name promises a change. Subscribers would have to filter on `previousZone !== newZone` themselves, which is the same logic phase 8 should encapsulate.
...
## ADR-033: Score state placement — top-level field on SimState

- **Date:** 2026-04-30
- **Decision:** `state.score` is a top-level field on `SimState`, typed `number`, initialized to `0` in `createSimState`. Not nested under `state.criticality`, not wrapped in a future `state.scoring` object. The field is populated by phase 8 (per ADR-032) and read by phases 1 (SCRAM) and 9 (meltdown/extinction) when emitting `runEnded`.
- **Context:** ADR-032 locked the score formula. Where `state.score` lives in the state tree is a separate question. There are three plausible homes: top-level on `SimState`, nested under `state.criticality` (since score depends on criticality zone), or nested under a future `state.scoring` object that anticipates richer scoring features. Each implies different things about future evolution.
- **Alternatives:**
  - *Nested under `state.criticality.score`* — rejected. Logical separation: score is *derived from* criticality but isn't part of criticality state. `state.criticality` holds k and zone — both are properties of the reactor's chain reaction. Score is a player-progression accumulator, scoped to the run, not to the reactor's instantaneous physics state. ADR-015 (state field semantics — fields have one meaning) generalizes here: `criticality` is one fact (the current reactor regime); score is a different fact (run-cumulative reward).
...
## ADR-035: Subscriber error handling — catch and continue

- **Date:** 2026-05-01
- **Decision:** When a subscriber's handler throws during `Sim.tick`'s dispatch, the error is caught, logged via `console.error('Sim subscriber error:', error, { event })`, and dispatch continues to the next subscriber and the next event. The sim does not halt, and other subscribers are not affected. The thrown error does not propagate out of `Sim.tick`.
- **Context:** The sim runs at 60Hz. A subscriber bug — most likely in renderer code under active development — must not be able to corrupt sim state or freeze the run. Phase 10 / `Sim.tick` is the only place where external code executes inside the sim's call stack; it's the natural quarantine point.
- **Alternatives:**
  - *Let the error propagate out of `Sim.tick`* — rejected. One renderer-side bug would crash the entire sim loop. Worse, because dispatch happens after `advanceTick`, the state has already advanced; throwing out of `Sim.tick` would leave the caller with a partially-progressed run and no clear recovery path. `console.error` plus continued dispatch keeps the sim itself sound.
  - *Catch and emit a meta-event for subscriber errors (e.g., `subscriberError`)* — rejected. Adds a new event type and a circular dispatch concern (what if the meta-event handler also throws?). No subscriber today needs to observe other subscribers' errors. If a future tooling subscriber needs that visibility, add it as a separate channel then.
  - *Allow subscribers to register a custom error handler* — out of scope for v1. The default behavior (`console.error` plus continue) is the right default; if a future caller wants structured logging or to suppress logs in headless balance scripts, that's a separate concern and a focused API addition (`Sim.onSubscriberError(handler)`) rather than something to design preemptively.
- **Consequences:** Each handler invocation in `Sim.tick` is wrapped in try/catch. On catch, dispatch continues to the next handler in the same set and then to the next event in `pendingEvents`. The sim's own state-clearing step (resetting `pendingEvents` to `[]`) runs unconditionally after dispatch — a thrown subscriber cannot leave events stuck in the queue. Edge case: if every subscriber for an event throws, the event is effectively lost (no one observed it successfully). Acceptable — that is a developer bug to fix, not a sim concern. `console.error` is dependency-free and works in Node and the browser; structured logging can be layered behind a custom error handler in v2 if needed.

## ADR-036: Visual polish deferred until mechanic validation

- **Date:** 2026-05-03
- **Decision:** Visual polish — sprite art, UI styling, animations beyond position updates, particle effects, screen shake, sound, font work, palette work, marketing visuals — is deliberately deferred until after the week-6 playtest gate. Until that gate passes, `packages/game` renders with primitive shapes (filled circles, plain rectangles, monospace HTML text) on a black background. Atom and neutron sprites are colored circles. There are no tweens, no particle emitters, no shaders, no audio. The HUD is plain HTML positioned over the canvas. The only state-driven visual changes are position updates and a small set of explicit alpha/border swaps tied to atom states (intact / excited / splitting / spent).
- **Context:** The risk we're managing is the classic solo-dev failure mode where polish lands before validation, the mechanic turns out not to hold attention, and the sunk cost in art makes the project hard to kill. We can also predict, ahead of any playtest data, that polished placeholders make playtest feedback worse: testers react to the production value rather than the loop. Holding the visual line at primitives keeps the next month of work cheap to throw away and forces the mechanic to carry itself.
- **Alternatives:**
  - *Polish in parallel with mechanic work* — rejected. Polish in this codebase is at least Aseprite assets, an animation pipeline, palette discipline, and HUD art direction. None of that is small. Doing it on a mechanic that may not survive playtesting is the failure mode CLAUDE.md was written to prevent.
...
## ADR-038: Prototype-mode extinction grace via config

- **Date:** 2026-05-03
- **Decision:** While the prototype is in pre-input evaluation mode, the sim's `endConditions.extinctionGracePeriod` is overridden to **1800 ticks (30 seconds)** at game-package config-load time. This gives a tester or the founder enough headroom to read the on-screen instructions, look at the playfield, and decide on a first action without the run terminating in 5 seconds (the production-default 300 ticks). The override is purely a config substitution — `packages/sim` is not touched, no game-side end-condition shim, no synthetic input, no "is this a tutorial?" branch in the sim. Once the Site system lands, real Site configs supply their own grace periods (the production default returns to 300 unless a Site explicitly overrides it).
- **Context:** First playable session showed the prototype was unevaluable: extinction fires almost immediately because k=0 every tick before the player learns what to click. The sim is correct — empty playfield with no fissions IS extinct — but it costs the session what it's there to do, which is "let a tester form an honest opinion of the mechanic over 3-5 minutes." We need a way to keep the run alive long enough for the player to engage. We considered three approaches: (1) skip ticks until first input, (2) game-side override of extinction logic, (3) extend grace via config. (1) and (2) require the game package to either gate the sim or override its end conditions, both of which violate the layer boundary in CLAUDE.md ("Sim never knows about Phaser. Presentation subscribes to events and sends commands."). (3) is the only approach that keeps the boundary clean.
- **Alternatives:**
  - *Pause sim ticks until first input* — rejected. Atoms never appear (fuel rod placement schedules releases on FUTURE ticks, which we'd be skipping), the rendering loop has nothing to draw, and the "what is this game" feedback loop the prototype is meant to test is broken before it starts. Solving the schedule problem would mean inventing a half-state where the sim runs but doesn't really run, which is a bug factory.
  - *Game-side override of run-end conditions* — rejected. The sim owns end semantics; having the game watch for `runEnded` and "veto" extinction events means the sim says one thing and the renderer says another. That's the kind of split-brain bug the architecture is set up to prevent.
  - *Game-side suppression by tracking ticks-before-first-input and offsetting `ticksBelowExtinction`* — rejected. Requires reading-and-rewriting sim state from outside, which the public API doesn't allow (state is `Readonly`) and shouldn't allow.
  - *Sim-side "tutorial mode" flag* — rejected. Polluting the sim with prototype-only concepts that vanish before launch is exactly the wrong direction. Config substitution is the same effect with no API surface change.
- **Consequences:** `packages/game/src/scenes/GameScene.ts` (or wherever the Sim is instantiated) clones the loaded default config and writes `endConditions.extinctionGracePeriod = 1800` before passing it to `new Sim(config, seed)`. Comment at the override site references this ADR so the prototype-only nature is unmistakable. Production Site configs will set their own grace period and the override goes away when the Site system lands. The longer grace period means a tester who wanders away from the keyboard for 25 seconds doesn't die, but also that a *genuinely* dead reactor takes a noticeable beat to confirm it — acceptable for prototype evaluation; tunable per-Site once shipping.
