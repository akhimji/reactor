...
- **Phases 5 and 6 — apply collisions and advance atom states (next major sim work)** — the natural follow-on to phase 4. Phase 5 reads `pendingNeutrons` from atoms in `excited` state and spawns the actual neutrons (random angles, `defaultNeutronSpeed` magnitude). Phase 6 owns the state-machine transitions: `excited → splitting` after one tick, `splitting → spent` after `splittingDuration` ticks. The exact relationship between phase 5's `pendingNeutrons` consumption and phase 6's `excited → splitting` transition is the load-bearing detail to resolve in that session — ADR-023 flags this as deferred. Phases 7-10 (auto-decay, criticality, end conditions, event emit) follow as a smaller follow-up session.
